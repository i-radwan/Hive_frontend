...
let settingsPanelViewModel = function (runningMode, shouter, state, gfxEventHandler, comm, logger) {
    ...
    self.connect = function () {
        if (runningMode() !== RUNNING_MODE.DESIGN) {
            shouter.notifySubscribers({text: "Connection is allowed only in design mode!", type: MSG_TYPE.ERROR}, SHOUT.MSG);
        ...
        try {
            ...
            comm.connect(self.ip(), self.port(), function () {
                shouter.notifySubscribers({text: "Connected to server!", type: MSG_TYPE.INFO}, SHOUT.MSG);
            });
        ...
        shouter.notifySubscribers({text: "Couldn't connect to the server!", type: MSG_TYPE.ERROR}, SHOUT.MSG);
